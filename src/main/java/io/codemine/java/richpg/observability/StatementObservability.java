 ...
 * call {@link #execute} exactly once to run the operation and end the span.
 */
public final class StatementObservability {
  ...
  static final AttributeKey<String> DB_OPERATION_NAME_KEY =
      AttributeKey.stringKey("db.operation.name");
  static final AttributeKey<String> DB_COLLECTION_NAME_KEY =
      AttributeKey.stringKey("db.collection.name");
  static final AttributeKey<String> STATEMENT_NAME_KEY =
   ...
   *
   * <p>Callers build this once per session and pass the same instance into every {@link
   * #forStatement} / {@link #forBatch} call, so that every statement execution in a session records
   * onto one shared instrument.
   *
  ...
  /**
   * Builds a {@code StatementObservability} bound to one batch of statements, starting its CLIENT
   * span.
   *
   * @param tracer the OpenTelemetry tracer used to create the CLIENT span
   * @param durationHistogram the shared duration histogram, from {@link #buildDurationHistogram}
   ...
   * @param representativeStatement a representative statement used for optional metadata attributes
   *     ({@code db.operation.name}, {@code db.collection.name})
  ...
  /**
   * Runs {@code operation} under this statement's already-started span, recording the {@code
   * db.client.operation.duration} histogram point and any slow-query warning, then ends the span.
   *
   * <p>Must be called exactly once per {@code StatementObservability}.
   *
