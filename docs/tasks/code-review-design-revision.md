...
- [ ] **Duplicated Code** — `SessionSettings`: the `withXxx` methods each
...
- [ ] **Duplicated Code (minor)** — `TransactionExecutor.
...
- [ ] **Speculative Generality** — `SqlStateClassifier.classify` handles
...
- [ ] **Mysterious Name (mild)** — `Session`'s field/param is still `config`
...
- [ ] **Doc rot (new)** — three javadocs cite the now-deleted plan:
  ...
  citations.
- [ ] **Doc rot (new)** — `Telemetry.startStatementOperation` javadoc says
...
- [ ] **Transaction span ended twice.** `Telemetry.finishTransactionOperation`
...
- [ ] **`isTransactionWide` inconsistent with `classify` on wrapped
...
- [ ] **`Session.execute` javadoc contradicts spec and itself** — says
